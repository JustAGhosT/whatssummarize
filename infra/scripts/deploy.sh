...
#
# Usage:
#   ./deploy.sh <environment> [--what-if] [--validate-only]
#
# Arguments:
...
deploy_infrastructure() {
    ...
    elif [ "$VALIDATE_ONLY" = "--validate-only" ]; then
...
main() {
    ...
    if [ "$WHAT_IF" != "--what-if" ] && [ "$VALIDATE_ONLY" != "--validate-only" ]; then
        post_deployment
        echo ""
    fi
...
# Show help
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "Usage: $0 <environment> [--what-if] [--validate-only]"
    echo ""
    echo "Arguments:"
    ...
    echo "  $0 prod --what-if         # Preview production changes"
